...
def find_vector_set(diff_image, new_size):
    ...
    elif diff_image.shape[2] >= 3:
        vector_set = np.zeros((int(new_size[0] * new_size[1] * new_size[2] / 75), 75))
...
def _find_FVS(EVS, diff_image, mean_vec, new):
    if len(diff_image) == 2:
        feature_vector_set = rearrange(diff_image,'(w s1) (h s2) -> (w h) (s1 s2)', s1=5, s2=5 )
        for i in range(1,5):
            diff_image_cache = np.roll(diff_image,i)
            for j in range(1,5):
                diff_image_cache2 = np.roll(diff_image_cache, j ,axis=1)[:-5, :-5]
                diff_image_cache2 = rearrange(diff_image_cache2, '(w s1) (h s2) -> (w h) (s1 s2)', s1=5, s2=5)
                feature_vector_set = np.concatenate((feature_vector_set, diff_image_cache2),axis=0)
    else:
        feature_vector_set = rearrange(diff_image,'(w s1) (h s2) c -> (w h) (s1 s2 c)', s1=5, s2=5)
        for i in range(1,5):
            diff_image_cache = np.roll(diff_image, i, axis=0)
            for j in range(1,5):
                diff_image_cache2 = np.roll(diff_image_cache,j, axis=1)[:-5,:-5,:]
                diff_image_cache2 = rearrange(diff_image_cache2, '(w s1) (h s2) c -> (w h) (s1 s2 c)', s1=5, s2=5)
                feature_vector_set = np.concatenate((feature_vector_set, diff_image_cache2),axis=0)
